...
from constants import (
    DREAMING_POKEMON_RESPONSE,
    PLAYSTATUS,
    SAVEDATA_DOWNLOAD,
    SLEEPILY_BITLIST,
    WAKE_UP_AND_DOWNLOAD,
    PUT_POKE_TO_SLEEP_RESPONSE,
...
def test_playstatus(client):
    ...
    assert (
        response.data == WAKE_UP_AND_DOWNLOAD
        or response.data == PUT_POKE_TO_SLEEP_RESPONSE
        or response.data == b"\x08"
    )


def test_worldbattle(client):
    response = client.get(
        "/dsio/gw", query_string={"gsid": "1", "p": WORLDBATTLE_DOWNLOAD}
    )
